...
#define CUSTOM_MODEL_PATH_SIZE 18	//Z:***INCREASE NUMBER FOR EACH MODEL!!!***
...
ModelIndex(iClient, iCaseNum, bool:bUsingMenu=false)
{
	...
	switch(iCaseNum)//1.4
	{
		...
		case 34:
		{
			if(!CheckForSameModel(iClient, LMCModelSectionType_Custom, view_as<int>(MODEL_HK416)))
				BeWitched(iClient, sCustomPaths[MODEL_HK416], false);
		...
		case 35:
		{
			if(!CheckForSameModel(iClient, LMCModelSectionType_Custom, view_as<int>(MODEL_UMP9)))
				BeWitched(iClient, sCustomPaths[MODEL_UMP9], false);
		...
		case 36:
		{
			if(!CheckForSameModel(iClient, LMCModelSectionType_Custom, view_as<int>(MODEL_G41)))
				BeWitched(iClient, sCustomPaths[MODEL_G41], false);
		...
		case 41:
		{
			if(!CheckForSameModel(iClient, LMCModelSectionType_Custom, view_as<int>(MODEL_M1903)))
				BeWitched(iClient, sCustomPaths[MODEL_M1903], false);
		...
		case 42:
		{
			if(!CheckForSameModel(iClient, LMCModelSectionType_Custom, view_as<int>(MODEL_UMP40)))
				BeWitched(iClient, sCustomPaths[MODEL_UMP40], false);
		...
		case 43:
		{
			if(!CheckForSameModel(iClient, LMCModelSectionType_Custom, view_as<int>(MODEL_TDASWEETMIGULOLI)))
				BeWitched(iClient, sCustomPaths[MODEL_TDASWEETMIGULOLI], false);
